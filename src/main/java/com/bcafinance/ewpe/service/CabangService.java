...
import com.bcafinance.ewpe.dto.*;
...
import com.bcafinance.ewpe.repo.CabangRepo;
...
@Service
public class CabangService implements IService<Cabang> {
    ...
    @Autowired
    public CabangService(CabangRepo cabangRepo, ModelMapper modelMapper) {
        strExceptionArr[0] = "CabangService";
        this.cabangRepo = cabangRepo;
        this.modelMapper = modelMapper;
    }

    @Override
    public ResponseEntity<Object> save(Cabang cabang, HttpServletRequest request) {
        if(cabang==null)
        {
        ...
        try{
            cabangRepo.save(cabang);

        }catch (Exception e)
        {
            strExceptionArr[1] = "save(Cabang cabang, HttpServletRequest request) --- LINE 59 \n"+ RequestCapture.allRequest(request);
    ...
    @Override
    public ResponseEntity<Object> update(Long id, Cabang cabang, HttpServletRequest request) throws Exception {
        Optional<Cabang> opCabang;
        Cabang cabangTrans;
        Boolean isValid = true;

        try{
            opCabang =  cabangRepo.findById(id);
            ...
            cabangTrans = opCabang.get();
            //UNTUK METHOD PATCH
            cabangTrans.setNamaCabang(cabang.getNamaCabang());
            cabangTrans.setKelas(cabang.getKelas());
            cabangTrans.setRegion(cabang.getRegion());
            cabangTrans.setNamaBM(cabang.getNamaBM());

            cabangRepo.save(cabangTrans);

        }
        catch (Exception e)
        {
            isValid = false;
            strExceptionArr[1] = "update(Long id, Cabang cabang, HttpServletRequest request) --- LINE 119 \n"+RequestCapture.allRequest(request);
            LoggingFile.exceptionStringz(strExceptionArr, e, OtherConfiguration.getFlagLoging());
            LogTable.inputLogRequest(logRequestRepo,strExceptionArr,e,OtherConfiguration.getFlagLogTable());
            return new ResponseHandler().generateResponse(
                    "Data tidak Valid",//message
    ...
    public ResponseEntity<Object> delete(Long id, HttpServletRequest request) {
        Optional<Cabang> cabangTrans =  cabangRepo.findById(id);

        if(cabangTrans.isEmpty())
        {
            return new ResponseHandler().generateResponse(
                    "Data tidak Valid",//message
                    HttpStatus.BAD_REQUEST,//httpstatus
    ...
    @Override
    public ResponseEntity<Object> saveBatch(List<Cabang> lt, HttpServletRequest request) {
        if(lt.isEmpty())
        {
            return new ResponseHandler().generateResponse(
                    "Data tidak Valid",//message
        ...
        try{
            cabangRepo.saveAll(lt);
        }catch (Exception e)
        {
            strExceptionArr[1] = "saveBatch(List<Cabang> lt, HttpServletRequest request) --- LINE 207 \n"+RequestCapture.allRequest(request);
            LoggingFile.exceptionStringz(strExceptionArr, e, OtherConfiguration.getFlagLoging());
            LogTable.inputLogRequest(logRequestRepo,strExceptionArr,e,OtherConfiguration.getFlagLogTable());
            return new ResponseHandler().generateResponse(
                    "Data Gagal Disimpan",//message
    ...
    public ResponseEntity<Object> findById(Long id, HttpServletRequest request) {
        Optional<Cabang> cabangTrans ;
        CabangDTO cabangDTO;
        try{
            cabangTrans  = cabangRepo.findById(id);
            if(cabangTrans==null)
            {
                return new ResponseHandler().generateResponse(
        ...
        cabangDTO = modelMapper.map(cabangTrans, new TypeToken<RegionalDTO>() {}.getType());
    ...
    public ResponseEntity<Object> findAllByPage(Integer page, Integer size, HttpServletRequest request) {
        Page<Cabang> pageCabang;
        List<CabangDTO> listCabangDTO;
        try
        {
            Pageable pageable = PageRequest.of(page,size, Sort.by("idCabang"));
            pageCabang = cabangRepo.findAll(pageable);
            int dataSize = pageCabang.getContent().size();
            if(dataSize==0)
            {
                return new ResponseHandler().generateResponse(
                        "Data tidak Ditemukan",//message
            ...
            listCabangDTO = modelMapper.map(pageCabang.getContent(), new TypeToken<List<CabangDTO>>() {}.getType());
        ...
        return new ResponseHandler().generateResponse(
                "Data Ditemukan",//message
                HttpStatus.OK,//httpstatus OK
                transformDataPaging.mapDataPaging(mapz,pageCabang,listCabangDTO),//object
                null,//errorCode diisi null ketika data berhasil disimpan
                request
        );
    }
    ...
    public ResponseEntity<Object> findAll(HttpServletRequest request) {
        ...
        List<CabangDTO> cabangDTOList;
        try{
            listCabang = cabangRepo.findAll();
            ...
            cabangDTOList = modelMapper.map(listCabang, new TypeToken<List<CabangDTO>>() {}.getType());
        }catch (Exception e)
        {
